...
本工作以 OpenLRM-Mix-Base-1.1 为研究对象，深入前馈架构的内部特征传播机制，首次量化揭示了编码器误差与解码器敏感度之间的**频域不匹配（Frequency-domain Mismatch）** 现象。基于该诊断，进一步提出与解码器频率传递函数相对齐的参数高效微调方法 **LoRA-FreqLoss**。核心发现：
